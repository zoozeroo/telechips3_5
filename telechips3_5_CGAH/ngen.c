...
// 타워 특성
#define ATTACK_TOWER_RANGE 80.0f // 타워 공격 범위
#define ATTACK_TOWER_DAMAGE 50   // 타워 공격력
...
typedef struct {
    TowerType type;
    float cooldown; // 공격 또는 자원 생산 쿨다운 타이머
} Tower;
...
void draw_game(ALLEGRO_BITMAP* background) {
    ...
    for (int i = 0; i < MAX_ENEMIES; ++i) {
        if (enemies[i].active) {
            al_draw_filled_rectangle(enemies[i].x - 5, enemies[i].y - 5, enemies[i].x + 5, enemies[i].y + 5, al_map_rgb(150, 50, 200));
        }
    }
    // ====== 공격 범위 시각화 ======
    ...
    // UI 그리기
    al_draw_textf(game_font, al_map_rgb(255, 255, 255), 10, 10, 0, "Caffeine: %d", game_state.caffeine);
    al_draw_textf(game_font, al_map_rgb(255, 255, 255), BUFFER_W - 10, 10, ALLEGRO_ALIGN_RIGHT, "LIVES: %d", game_state.lives);
}
...
int main(void) {
    ...
    bool running = true;
    bool redraw = true;
    double last_enemy_spawn_time = al_get_time();
    ...
    while (running) {
        ...
        case ALLEGRO_EVENT_TIMER:
            ...
            // 2초마다 적 생성
            if (al_get_time() - last_enemy_spawn_time > 2.0) {
                spawn_enemy();
